...
const playbooksController = () => {
  return {
    ...
    GET_ONE: function* () {
      ...
      const {id, name, description, company_id, doc, assigned, submitted_doc, current_status, percent_submitted, userId, username, first_name, last_name, is_admin, rolename } = result[0];
      this.status = 200;
      this.body = {
        playbook: {id, name, description, company_id, doc, assigned, submitted_doc, current_status, percent_submitted},
        users: [{ userId, username, first_name, last_name, is_admin, rolename }]
      };
    },
